...
class ExcelExtractorGesamtuebersicht:
    ...
    def read_all(self):
        ...
        for ws in all_relevant_work_sheets:
            ...
            data = extract_all_data_from_sheet(self, ws)
...
def _teacher_sanity_checks(all_teachers_dict, all_teachers_list, all_classes):
    ...
    for teacher_obj in all_teachers_list:
        ...
        if availability_preference_table is None:
            print(
                f"warning: teacher '{key}' [{teacher_obj['teacher_full_name']}] [{teacher_obj['contract_form']}] has no availability preferences -> teacher will not be used!")
            ...
            for slot_index, slot_obj in enumerate(availability_preference_list):
                ...
                if prefilled_class is not None:
                    ...
                    if not is_known_class:
                        print(
                            f"warning: teacher '{key}' [{teacher_obj['teacher_full_name']}] has pref for unknown class '{prefilled_class}' on day index '{day_index}' slot '{slot_index}' -> slot will be set to skip/not allowed and class removed because invalid")
