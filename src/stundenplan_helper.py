...
class StundenplanHelper:
            ...
            for day_index, table_info_list in enumerate(table_dates):
                for slot_index, info_obj in enumerate(table_info_list):
                    ...
                    if should_not_process:
                        var_names = get_all_vars_with_preset(self.all_var_names, class_key=class_key, day_index=day_index, slot_index=slot_index)
                        if len(var_names) == 0:
                            print("")
        ...
        for teacher_obj in self.excel_extractor.all_teachers_list:
            ...
            for day_index, day_slots_list in enumerate(availability_preference_table):
                for slot_index, slot_obj in enumerate(day_slots_list):
                    class_key = slot_obj['class_name']
                    allowed = slot_obj['allowed']

                    if not allowed:
                        var_names = get_all_vars_with_preset(self.all_var_names, teacher_key=teacher_key, class_key=class_key,
                        ...
                        var = [self.all_vars[var_name] for var_name in var_names]
                        self.problem += lpSum(var) == 0, f"skip not allowed teacher slots {var_names}, {day_index}, {slot_index}"
    ...
    def setup_constraints(self):
        ...
        for day_index in range(self.max_days):
            for class_obj in self.excel_extractor.all_classes:
                ...
                var = [self.all_vars[var_name] for var_name in var_names]
                c1 = lpSum(var) <= self.max_slots_per_day, f"every day can only have a max number of slos for one class {c_count}"
                c_count += 1
                self.problem += c1
        ...
        for teacher_obj in self.excel_extractor.all_teachers_list:
            ...
            for day_index in range(self.max_days):
                for slot_index in range(self.max_slots_per_day):
                    var_names = get_all_vars_with_preset(self.all_var_names, teacher_key=teacher_key, slot_index=slot_index,
                                                         day_index=day_index)
                    var = [self.all_vars[var_name] for var_name in var_names]
        ...
        c_count = 0
        for class_obj in self.excel_extractor.all_classes_list:
            class_key = class_obj['key']
            for day_index in range(self.max_days):
                for slot_index in range(self.max_slots_per_day):
    ...
    def solve_timetable_problem(self):
        self.problem.writeLP("StundenplanReal.lp")
        self.problem.solve()
...
def get_all_vars_with_preset(all_var_names, teacher_key=None, subject_key=None, day_index=None, slot_index=None, class_key=None):
